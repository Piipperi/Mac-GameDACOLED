...
Vibe coded Apple Silicon native macOS menu bar app for SteelSeries OLED devices such as the Nova Pro's GameDAC. It uses SteelSeries' local GameSense API, so it might work on other SS peripherals with OLED displays as well. Expect a lot of bugs that I may or may not fix in the future.
